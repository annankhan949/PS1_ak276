...
```{julia}
# -- Compute posterior return levels at 50 and 100 years --
T_years = [50, 100]
quantiles = 1 .- 1 ./T_years  # e.g., [0.98, 0.99]

posterior_rl_samples = [quantile.(posterior_GEVs, q) for q in quantiles]
...
summary_df = DataFrame(
    ...
    Mean = [mean(samples) for samples in posterior_rl_samples],
    Lower90 = [quantile(samples, 0.05) for samples in posterior_rl_samples],
    Upper90 = [quantile(samples, 0.95) for samples in posterior_rl_samples]
)

println("Posterior return level estimates with 90% credible intervals:")
summary_df

fig_post_rl = let
    fig = Figure(size=(800, 500))
    ax = Axis(fig[1, 1],
        title="50-year and 100-year Return Levels (Posterior)",
        xlabel="Return Period (years)",
        ylabel="Return Level (inches)",
        xscale=log10,
        xticks=T_years
    ...
    scatter!(ax, T_years, summary_df.Mean, color=:blue, markersize=10, label="Posterior Mean")

    # Add 90% CI error bars
    for (i, T) in enumerate(T_years)
        y_low = summary_df.Lower90[i]
        y_high = summary_df.Upper90[i]
        lines!(ax, [T, T], [y_low, y_high], color=:blue, linewidth=2)
    end
